...
// Function to create semester-specific other_subject table if it doesn't exist
function createSemesterOtherSubjectTable($conn, $semester) {
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if (isset($_GET['check_code'])) {
        ...
        // Check across all semester tables for both regular and other subjects
        $semesters = ['III', 'IV', 'V', 'VI'];
        $is_available = true;
        
        // First check in regular subject tables
        ...
        // Then check in other_subject tables if still available
        if ($is_available) {
            foreach ($semesters as $semester) {
                $table_name = "sem{$semester}_other_subjects";
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'list') {
        ...
        foreach ($semesters_to_check as $curr_semester) {
            $table_name = "sem{$curr_semester}_other_subjects";
            
            // Ensure table exists
            createSemesterOtherSubjectTable($conn, $curr_semester);
...
if ($_SERVER['REQUEST_METHOD'] == 'DELETE' || (isset($_GET['action']) && $_GET['action'] == 'delete')) {
    ...
    if ($subject_id) {
        ...
        if ($deleted) {
            $response = array(
                'status' => 'success',
                'message' => 'Additional subject deleted successfully'
            );
        } else {
            $response = array(
                'status' => 'error',
                'message' => 'Additional subject not found or could not be deleted'
            );
        }
...
// Check if form is submitted for adding an additional subject
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!createSemesterOtherSubjectTable($conn, $semester)) {
        $response['status'] = 'error';
        $response['message'] = "Error creating semester additional subjects table.";
        echo json_encode($response);
        exit;
    }
    ...
    // First check if subject code already exists in any semester table (both regular and additional)
    $semesters = ['III', 'IV', 'V', 'VI'];
    $code_exists = false;
    
    // Check in regular subject tables
    ...
    // Check in other subject tables if not found in regular tables
    if (!$code_exists) {
        foreach ($semesters as $check_semester) {
            $check_table = "sem{$check_semester}_other_subjects";
    ...
    $sql = "INSERT INTO $table_name (subject_name, subject_code, subject_type, credits) 
            VALUES (?, ?, ?, ?)";
    ...
    if ($stmt->execute()) {
        ...
        $response['message'] = "Additional subject added successfully.";
    } else {
        $response['status'] = 'error';
